		 	    ...
		 	    for(var k in file){
		 	    	if(file[k] instanceof  File){
				 	    ...
				 	    {		 		    	  
			 			    	...
			 			    	formData.append("x:tenantid",tenantid)//租户
			 			    	formData.append("x:groupname",groupname)//分组名称 自定义参数的传法
			 			    	formData.append("x:filepath",filepath)//单据信息   自定义参数的传法
			 			    	formData.append("x:permission",permission)//共有还是私有
			 			    	formData.append("x:modular",paramers.modular)//模块信息
			 			    	formData.append("x:sysid",sysid)//系统id
			 			    	formData.append("x:userid",userid)//用户
			 ...
			 function send_request(obj,paramers,callback){
			 	$.ajax({
		            ...
		            data : {					            	   
						...
						groupname : paramers.groupname,//分组名称
						filepath : paramers.filepath,//唯一标示
						filename : paramers.filename,//附件名称
						userid : paramers.userid,
						sysid : paramers.sysid,
			        },
