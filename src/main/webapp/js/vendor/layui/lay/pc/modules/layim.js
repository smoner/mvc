...
var   openuserid = getQueryString('touserid');
...
layui.define(['jquery', 'layer', 'laytpl', 'upload'], function(exports){
  ...
  //添加好友/群
  LAYIM.prototype.friendstatus = function(data){
    return friendstatus(data), this;
  };  
  
  //在线用户排序
  var sortusername=function () {
    var oUl = $(".layui-layim-list");
     var aLi = oUl.get(1).getElementsByTagName("li");  
    var arr = [];  
    var i = 0;  
    //1、转成数组  
    for (i = 0; i < aLi.length; i++) {  
        arr[i] = aLi[i].getElementsByTagName("span").innerHTML();  
    }  
    //console.log(arr)  
    //2、数组排序  
    arr.sort();
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" {{data.status === "offline" ? "class=gray" : ""}} data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'<ul class="layui-layim-tool">'
      ...
      ,'{{# if(d.base.find){ }}'
      //,'<li ayim-event="find" title="查找">&#xe61f;</li>'
      ,'{{# } }}'
      ,'{{# if(!d.base.copyright){ }}'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ,'<div class="layim-chat-title">'
      ,'<a class="layim-chat-other {{d.data.status === "offline" ? \"gray\" : \"\"}}" id="layim-title-{{d.data.id}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" id="imgfile" name="file"></span>'
        ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" id="uploadfile" name="file"></span>'
         ,'{{# }; }}'
        ,'{{# if(d.base && d.base.chatLog){ }}'
        ,'<span class="layim-tool-log" layim-event="chatLog"><i class="layui-icon">&#xe60e;</i>聊天记录</span>'
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="layim-chat-textarea"><div contentEditable="true"></div></div>'
  ...
  layui.laytpl.date = function(timestamp){
    return new Date(timestamp||new Date()).Format("yyyy-MM-dd hh:mm:ss");
  ...
  layui.laytpl.content = function(content){
    ...
    var html = function(end){
      return new RegExp('\\n*\\['+ (end||'') +'(pre|div|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5|img|a)([\\s\\S]*?)\\]\\n*', 'g');
    };
  ...
  var elemChatMainSysNotice = ['<li>'
    ,'<div class="layim-chat-notice">{{ layui.laytpl.content(d.content||"&nbsp") }}</div>'
  ,'</li>'].join('');
  
  var elemChatList = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this {{d.data.status === "offline" ? "gray" : ""}}"  layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var post = function(options, callback, tips){
    ...
    return $.ajax({
      ...
      ,success: function(res){
        res.code == 0 
          ? callback && function(){
            //modify by songhlc change img url to/avatar if not start with http
            layui.each(res.data.friend,function(i,data){
              layui.each(data.list,function(j,_data){
                _data.avatar = _data.avatar.indexOf("http")==0?_data.avatar:(cache.base.baseurl+'/'+_data.avatar)
              })
            });
            layui.each(res.data.group,function(i,_data){              
                _data.avatar = _data.avatar.indexOf("http")==0?_data.avatar:(cache.base.baseurl+'/'+_data.avatar)
            });
            res.data.mine.avatar = res.data.mine.avatar.indexOf("http")==0?res.data.mine.avatar:(cache.base.baseurl+'/'+res.data.mine.avatar)            
            callback(res.data||{})
          }()//callback(res.data||{})
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    post(options.init, function(data){
      ...
      if(openuserid||options.touserid){
          var touserid =  openuserid||options.touserid;
          layui.each(obj.friend, function(index1, item1){
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layui-layim-list').on('contextmenu', 'li', function(e){
      var othis = $(this);
      var html = '<ul data-id="'+ othis[0].id +'" data-index="'+ othis.data('index') +'"><li layim-event="menufriend" data-type="one">添加至常用联系人</li></ul>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,shift: 2
      ,offset: 'rb'
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'友企通') +'</span>'
  ...
  var changeChat = function(elem, del){
    ...
    if(elem.attr('class')){
        ...
        //layim-chatlist-friendliangjfc
        var id = classname.substring(21);
        //如果存在提醒则去掉
        layui.each(messageanim, function(index, item){          
            if(item[0]==id){
                var pingTimerID = item[1];
                if (pingTimerID) clearInterval(pingTimerID);
                messageanim.splice(index, 1); //从提醒列表里删除
                return true;
            }
        });
    }
  ...
  var thisChat = function(){
    ...
    return {
      elem: cont
      ,data: to
      ,textarea: cont.find('.layim-chat-textarea div')
      ,selection:selection
      ,range:range
    };
  };
  ...
  var sendMessage = function(){
    ...
    data.content = $(thatChat.textarea).html();
  ...
  var getMessage = function(data){
    ...
    var notice=false;
    if(data.content.indexOf("[系统提示]")>-1){
        notice=true;
    }
    ...
    if(thatChat.data.type + thatChat.data.id !== data.type + data.id){
      ...
      layui.each(messageanim, function(index, item){          
          if(item[0]==data.id){
              ...
              tabmessage[0]=data.id;              
              var pingTimerID = setInterval(animMessage, 3000,elem);
              tabmessage[1]=pingTimerID;
              messageanim[index] = tabmessage;
              had = true;
              return true;
          }
      });
      if(!had){
            ...
            var pingTimerID = setInterval(animMessage, 3000,elem);
            tabmessage[1]=pingTimerID;
            messageanim[messageanim.length] = tabmessage;
      }      
    }
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      if(notice){
        ul.append(laytpl(elemChatMainSysNotice).render(data));
      } else{ 
        ul.append(laytpl(elemChatMain).render(data));
      }
    }
  ...
  //好友上线、离线处理
  var friendstatus = function(data){
    var listElem = layimMain.find('.layim-list-friend');
    var obj = {};
    if(cache['friend']){
        //最后一个在线好友索引位置
        var onlineindex = 0;
        var liid ="#layim-friend"+data.id;
        var hasoffline = false;
        $(liid).parent().children().each(function(index, item){
            if(item.className=="gray") {
                onlineindex = index==0 ? 0: index-1;
                hasoffline=true;
                return true;
            }
        });
        if(!hasoffline){
            onlineindex=$(liid).parent().children().length-1;
        }
        layui.each(cache.friend, function(index1, item1){
          layui.each(item1.list, function(index, item){
            if(data.id == item.id){
              var li = listElem.find('>li').eq(index1);
              var fdata = cache.friend[index1].list[index];  
              var fli = li.find('.layui-layim-list>li').eq(index).clone();
              var chatlist = $(".layim-chatlist-friend"+item.id);
              var chattitle = $('#layim-title-'+item.id);
              if(data.status=="online"){                  
                  fli.removeClass("gray");
                  chatlist.removeClass("gray");
                  chattitle.removeClass("gray");
                  fdata['status']="online";
                  cache.friend[index1].list.splice(0, 0, fdata); //把新好友插入到第一行
                  cache.friend[index1].list.splice(index+1, 1); //从cache的friend里面也删除掉好友  
                  li.find('.layui-layim-list>li').eq(0).before(fli);//在第一位插入一条新li
                  li.find('.layui-layim-list>li').eq(index+1).remove();//同时删除下一索引的数据
              } else {
                  fli.addClass("gray");
                  chatlist.addClass("gray");
                  chattitle.addClass("gray");
                  fdata['status']="offline";
                  cache.friend[index1].list.splice(onlineindex+1, 0, fdata); //把离线的人插入到离线位置第一位
                  cache.friend[index1].list.splice(index, 1); //从cache的friend里面也删除掉好友   
                  li.find('.layui-layim-list>li').eq(onlineindex).after(fli);
                  li.find('.layui-layim-list>li').eq(index).remove();
              }
              return true;
            }
  ...
  var focusInsert = function(obj, str){    
    var thatChat = thisChat();
    var range=thatChat.range;
    var doc = obj;
    obj.focus();  
    var contenthtml = layui.laytpl.content(str);
    var bowser = checkBrowser();
    if(contenthtml.startWith('<img')||contenthtml.startWith('<a')){
        //var img = $(contenthtml);
        obj.focus();
        /*
        if(range!=null){ try {range.move("character");range.select()}  catch(e){} };
        document.execCommand("InsertImage", false,img.attr("src"));
        $(obj).find('img').each(function(index,imgitem){
           var imageitem =$(imgitem);
           if(!imageitem.hasClass('layui-layim-photos')){
               imageitem.addClass('layui-layim-photos');
           }
        });*/
        if(bowser.startWith('IE')){
            document.execCommand("pasteHTML", false,contenthtml);
        } else {
            document.execCommand("insertHTML", false,contenthtml);
        }        
        obj.focus();
    } else{
         document.execCommand("InsertText", false, contenthtml);
    }   
  };
  ...
  var anim = 'layui-anim-up', events = {
    ...
    //关于
    ,about: function(){
      layer.alert('版本： 1.0 <br>版权所有：<a href="http://www.yonyou.com" target="_blank">用友网络科技术股份有限公司</a>', {
        title: '关于友企通'
        ,shade: false
      });
    }
    ...
    ,image: function(othis){
      ...
      var result=$.ajax({url:'im/osssign' + param,async:false});
      console.log(result);
      ...
      layui.upload({
        ...
        //file input放到末位
        ,file: othis.find('input[type=file]')[0]
        ,unwrap: true
        ,check: type
        ,success: function(res){
          /*try {
          ...
          if(res.code == 0){
            res.path = res.path || {};
            if(type === 'images'){
              focusInsert(thatChat.textarea[0], 'img['+ (upload.url+"/"+res.path||'') +']');
            } else if(type === 'file'){
              focusInsert(thatChat.textarea[0], 'file('+ (upload.url+"/"+res.path||'') +')['+ (res.filename||'下载文件') +']');
            }
          } else {
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,content: cache.base.chatLog + '&chatid=' + thatChat.data.id + '&type=' + thatChat.data.type
      });
    }
    ...
    ,menuHistory: function(othis, e){
     ...
     //添加至常用联系人
    ,menufriend: function(othis, e){
      var local = layui.data('layim')[cache.mine.id] || {};
      var parent = othis.parent(), type = othis.data('type');
      sortusername();
      layer.closeAll('tips');
    }
    ,setting: function(othis, e){
     layer.open({
        type: 2
        ,title: '系统设置'
        ,shade: false
